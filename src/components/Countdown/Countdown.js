...
export default function Countdown({ toDate }) {
    ...
    return (
        <>{Object.entries(formatedCountdown).every(([_, value]) => {
                return value >= 0
            }) && 
            <div className="flex gap-5 mt-7">   
                    <div className="flex flex-col justify-center items-center">
                        <h3 className="text-lg text-emerald-500">Days</h3>
                        <div className="flex flex-row gap-2">
                            <CountdownCard previousNumber={Math.floor(previousFormatedCountdown.days / 10)} newNumber={Math.floor(formatedCountdown.days / 10)} count={count}/>
                            <CountdownCard previousNumber={previousFormatedCountdown.days % 10 } newNumber={formatedCountdown.days % 10} count={count}/>
                        </div>
                    </div>
                    <div className="flex flex-col justify-center items-center">
