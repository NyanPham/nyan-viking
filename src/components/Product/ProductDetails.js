...
import backpackSizeChart from '../../assets/backpack-size-chart.png'
    ...
    useEffect(() => {
        ...
        if (viewedCurrentProduct == null) {
            setRecentlyViewed((prevRecentlyViewed) => {
                const newRecentlyViewed = [...prevRecentlyViewed, currentProduct]
                while (newRecentlyViewed.length > 5) {
    ...
    return (
        <section className="py-7 px-12 w-full min-h-screen flex flex-row gap-8">
            <div className="w-3/5  rounded-sm flex flex-col items-center">
                ...
                <div className="mt-12">
                    ...
                    <p className="text-md leading-8 mt-4">
                        {currentProduct?.description} Lorem ipsum dolor sit amet, consectetur adipisicing elit. Soluta
                        repudiandae libero corrupti quam consequuntur, iste rem facilis nihil quibusdam iusto, beatae
                        animi possimus aut, molestias dignissimos sapiente consequatur doloribus inventore. Dolore
                        accusamus et molestias laboriosam cum nisi, perferendis veniam officiis, corporis magni
                        asperiores a. Odit, neque tenetur doloribus doloremque odio corrupti voluptate et vel in maiores
                        dolor alias autem nostrum.
                    </p>
                </div>
                <div className="mt-12">
            ...
            <div className="w-2/5 sticky h-max top-20 left-0">
                ...
                <h1 className="text-2xl font-semibold uppercase text-gray-900 tracking-wider">
                    {currentProduct?.title}
                </h1>
                {currentProduct?.salePercent > 0 ? (
                    <div className="flex flex-row gap-5 items-center">
                        <h3 className="text-base font-normal uppercase line-through text-gray-600 tracking-wider mt-5">
                            {currentProduct?.price ? formatPrice(currentProduct.price, 'USD') : 'Free'}
                        </h3>
                        <h3 className="text-xl font-semibold uppercase text-emerald-500 tracking-wider mt-5">
                            {currentProduct?.price
                                ? formatPrice(
                                      currentProduct.price - currentProduct.price * (currentProduct.salePercent / 100),
                                      'USD'
                                  )
                                : 'Free'}
                        </h3>
                    ...
                    <h3 className="text-xl font-normal uppercase text-gray-600 tracking-wider mt-5">
                        {currentProduct?.price ? formatPrice(currentProduct.price, 'USD') : 'Free'}
                    </h3>
                )}
                <div className="flex flex-row mt-5 gap-5 items-center relative after:content-[''] after:absolute after:-bottom-5 after:left-0 after:w-full after:h-px after:bg-gray-300">
                        ...
                        <div className="flex flex-row space-x-1">
                            {Array(currentProduct.rating)
                                ...
                                .map((_, index) => (
                                    <FontAwesomeIcon
                                        icon={faStar}
                                        key={`great_star_${index}`}
                                        className="text-gray-900"
                                    />
                                ))}
                            {Array(5 - currentProduct.rating)
                                .fill()
                                .map((_, index) => (
                    ...
                    <p className="grow text-gray-900 font-bold cursor-pointer underline underline-offset-1 decoration-2 hover:text-gray-500 transition">
                        Write a review
                    </p>
                    <FontAwesomeIcon
                        icon={faUpload}
                        className="text-gray-900 p-3 rounded-full bg-gray-200 cursor-pointer hover:bg-gray-100 hover:text-gray-500 transition"
                    />
                </div>
                ...
                <form className="mt-10" onSubmit={handleAddToCart}>
                    ...
                    <div className="flex flex-row gap-3 mt-3">
                        {currentProduct?.colors.map((color, index) => (
                            <div
                                key={`color_${index}`}
                                className={`${
                                    selectedColor === color ? 'after:scale-100' : 'after:scale-0'
                                } relative z-10 w-7 h-7 ${
                                    COLOR_MAP[color]
                                } rounded-full inline-block cursor-pointer after:content-[''] after:absolute after:top-1/2 after:left-1/2 after:-translate-y-1/2 after:-translate-x-1/2 after:w-9 after:h-9 after:border after:border-gray-900 after:rounded-full after:-z-0 after:transition after:duration-300`}
                    ...
                    <div className="flex flex-row gap-3 mt-3">
                        {currentProduct?.sizes.map((size, index) => (
                            <div
                                key={`size_${index}`}
                                className={`${
                                    selectedSize === size ? 'bg-gray-900 text-white' : 'text-gray-700'
                                } w-12 h-10 border border-gray-300 flex justify-center items-center text-sm rounded-lg cursor-pointer transition duration-300`}
                    ...
                    <div className="flex flex-row mt-5">
                        <button
                            className="h-9 w-9 border border-gray-300 rounded-l-sm flex justify-center items-center text-xl"
                            type="button"
                        ...
                        <input
                            type="text"
                            className="w-16 h-9 text-center border-t border-b border-gray-300 flex justify-center items-center text-lg"
                        ...
                        />
                        <button
                            className="h-9 w-9 border border-gray-300 rounded-r-sm flex justify-center items-center text-xl"
                            type="button"
                            onClick={handleAmountClick}
                            data-action="add"
                        >
                    ...
                    <div className="flex flex-row mt-5 gap-2">
                        {currentUser?.uid ? (
                            <button
                                ...
                                type="submit"
                                disabled={loading || !currentProduct?.amountInStock}
                            >
                                {currentProduct?.amountInStock > 0 ? (loading ? 'Adding' : 'Add') : 'Out Of Stock'}
                            ...
                            <Link to="/login" state={{ hasPreviousPage: true }} className="submit-btn" type="button">
                                {currentProduct?.amountInStock > 0 ? 'Add' : 'Out Of Stock'}
                                {currentProduct?.amountInStock > 0 && (
                                    <FontAwesomeIcon icon={faCartShopping} className="ml-2" />
                                )}
                            </Link>
                    ...
                    {Object.keys(errors).map((key, index) => (
                        <p
                            key={`adding_error_${index}`}
                            className={`${index === 0 ? 'mt-2' : ''} text-sm text-red-600`}
                        >
                            {errors[key]}
                        </p>
                    ))}
