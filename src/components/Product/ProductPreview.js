...
export default function ProductPreview(product) {
    const { docId, code, imageURL, title, description, price, colors, tags, categories, sizes, salePercent } = product

    const elementRef = useRef()
    const inView = useOnScreenOnce(elementRef, '30px')

    return (
        <div
            className={`${
                inView ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'
            } transform transition duration-1000 px-4 py-3 card w-56 h-72 bg-white cursor-pointer relative overflow-clip group`}
            ref={elementRef}
        >
            ...
            <div className="flex flex-col justify-between items-center h-full">
                <div className="w-32 h-32 mt-5 relative flex justify-center items-center">
                    <img src={imageURL} alt={title} className="max-w-full max-h-full object-cover object-center" />
                </div>
                <div className="w-full">
                    <h3 className="text-center text-gray-900 text-lg font-semibold">{title}</h3>
                    <p className="text-center text-gray-900 italic text-base w-full truncate">{description}</p>
            ...
            <div className="absolute inset-0 bg-transparent flex justify-center items-center pointer-events-none group-hover:bg-sky-900/30 group-hover:pointer-events-auto transform transition duration-500">
                <Link to={`/product/${docId}`} className="px-5 py-3 bg-white rounded-sm -rotate-180 scale-0 group-hover:scale-100 group-hover:rotate-0 transform transition duration-500">
                    Read Details
                </Link>
            </div>
        </div>
